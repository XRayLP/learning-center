...
class FormFileUpload extends \Widget implements \uploadable
{
    ...
    public function validate()
    {
        ...
        //File is bigger than the free space
        if(isset($arrSpace))
        {
            $usedSpace = FileManager::getUsedDirSpace($this->User->homeDir);
            $neededSpace = $usedSpace + $file['size'];
            ...
            if ($maxSpace < $neededSpace) {
                $this->addError(sprintf($GLOBALS['TL_LANG']['ERR']['usedSpace'], $freeSpace));
                unset($_FILES[$this->strName]);
                return;
            }
        }
        ...
        if (!$this->hasErrors())
        {
            ...
            if ($this->storeFile)
            {
                ...
                if ($this->useHomeDir && FE_USER_LOGGED_IN)
                {
                    ...
                    if(isset($fid)) {
                        $multiSRC = \FilesModel::findById($fid)->uuid;
                    } else {
                        $multiSRC = $this->User->homeDir;
                    }

                    if ($this->User->assignDir && $this->User->homeDir)
                    {
                        $intUploadFolder = $multiSRC;//$this->User->homeDir;
                    }
                }
